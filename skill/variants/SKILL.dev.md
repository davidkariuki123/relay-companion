...
pauses each member's agent on that topic until they approve the new text in the
app. Reading a topic changes nothing for anyone; the person's own open of the
board is the only read watermark.

The hook context lists the person's topics with their mandates at the start of
a session and announces new posts, invitations and mandate changes afterwards.
When the current work falls under a subscribed topic's mandate, read the board
with relay_topic_fetch before assuming what other members are doing, and pass
since from the hook record to get only what is new. Use what you learn
