...
test("Task lifecycle tools are developer-only and bind work to the calling agent session", async () => {
  const developerNames = new Set(toolsForAccount({ requests:true, aiSessions:true, connectors:true }).map((tool) => tool.name));
  const ordinaryNames = new Set(toolsForAccount({ requests:false, aiSessions:false, connectors:false }).map((tool) => tool.name));
  ...
  assert.deepEqual(calls, [
    ["start", "relay_task_1", {
      ...
      taskRunOwner: { kind: "external_mcp", provider: "codex", ...(native ? { nativeSessionId: native } : {}) },
    }],
    ["complete", "relay_task_1", {
      forHuman: "Done and verified.",
      forAgent: "The focused test suite passed.",
      attachments: [],
      idempotencyKey: "finish_task_1",
...
test("ordinary Relay MCP directs Claude and Codex to the membership-scoped Granular employee contact", () => {
  ...
  assert.match(agentDescription, /massively more costly/i);
...
test("relay_message_edit keeps the MCP-only human-writing review", async () => {
  ...
  const args = {
    relayId: "relay_1", forHuman: Array.from({ length: 121 }, (_, i) => `word${i}`).join(" "),
    idempotencyKey: "edit_review_1", longForHumanConfirmed: true,
  };
  await assert.rejects(handleCall(client, "relay_message_edit", args, { mode: "messages-only" }), /review threshold is 120 words/);
...
test("relay_chat_reply reviews an overlong human message before reading or sending", async () => {
  ...
  const args = {
    chatId: "chat_a",
    forHuman: Array.from({ length: 121 }, (_, index) => `reply${index + 1}`).join(" "),
  ...
  await assert.rejects(
    ...
    /review threshold is 120 words[\s\S]*Nothing was sent/i,
  );
...
test("relay_send requires an exact second review for human messages over 120 words", async () => {
  assert.equal(FOR_HUMAN_SOFT_WORD_LIMIT, 120);
  assert.equal(FOR_HUMAN_TYPICAL_WORD_LIMIT, 120);
  ...
  const forHuman = Array.from({ length: 121 }, (_, index) => `word${index + 1}`).join(" ");
  const args = {
    recipient: { relayUserId: "usr_sven" },
    kind: "message",
  ...
  await assert.rejects(
    ...
    /forHuman is 122 words[\s\S]*Nothing was sent/i,
  );
...
test("long-message review state never crosses broker connections", async () => {
  ...
  const args = {
    ...
    kind: "message",
    title: "Read receipt design",
    forHuman: Array.from({ length: 121 }, (_, index) => `word${index + 1}`).join(" "),
    forAgent: "The complete technical document.",
    longForHumanConfirmed: true,
...
test("relay_send accepts the 120-word review boundary without making it a target", async () => {
  const calls = [];
  const client = {
    async sendRelay(payload) {
      calls.push(payload);
  ...
  const forHuman = Array.from({ length: 120 }, (_, index) => `word${index + 1}`).join(" ");
  await handleCall(client, "relay_send", {
    recipient: { relayUserId: "usr_sven" },
    kind: "message",
    title: "Review boundary proof",
    forHuman,
    forAgent: "This payload proves that exactly 120 forHuman words remain within the boundary.",
...
test("the send path is annotated anthropic/alwaysLoad so no serving mode defers it", () => {
  ...
  for (const developer of [true, false]) {
    ...
    for (const tool of tools) {
      if (expectedAlwaysOn.has(tool.name)) continue;
...
test("a mint runs the 120-word review before it reads a single file", async () => {
  const draft = Array.from({ length: 130 }, (_, index) => `word${index}`).join(" ");
  let minted = 0;
  const fakeClient = {
    async mintShareLink() {
      minted += 1;
