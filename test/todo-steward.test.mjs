...
test("the brief carries the person, general evidence discovery and optional board leads", () => {
  ...
  assert.match(prompt, /Use only triage, in_progress and done/);
  assert.match(prompt, /Deferred or waiting obligations stay in triage/);
  assert.match(prompt, /Use only triage, in_progress and done/);
  ...
  assert.doesNotMatch(prompt, /backlog/i);
