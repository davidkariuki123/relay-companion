...
function buildContext(snapshot, { firstPrompt, newItems }) {
  ...
  const lines = firstPrompt
    ? [
        ...
        "A record with \"message\" is a typed text shown in full: it IS the entire Relay, so use it directly, speak of it as a message from its sender, and never call relay_inbox_list just to read one (only a message ending in … is truncated and worth opening). A record with \"title\" names a larger Relay: open any likely to improve the current request with relay_inbox_list({relayIds:[...]}), without asking first, and use it as background.",
    ...
    : [
        ...
        "A NEW record with \"message\" is a typed text shown in full — the entire Relay. If it is relevant to the current session's work, use it and refer to it as a message from its sender; open nothing (only a message ending in … is truncated and worth opening). A NEW record with \"title\" names a larger Relay: if relevant, open it immediately with relay_inbox_list({relayIds:[...]}) without asking, then tell the human who sent it, its title, and the useful gist. If a NEW record is not relevant to the current work, do not open it and do not mention it; continue the task. Never open or use a Relay's content without telling the human you did.",
