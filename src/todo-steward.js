...
export const STEWARD_ATTENTION_STATUSES = Object.freeze(["triage", "in_progress"]);
...
export function buildStewardPrompt({
  ...
  return [
    ...
    "1. Set status and evidence with relay_todo_update. Use triage for important open obligations, in_progress for actual live work, and done for concluded obligations or information that never requested action. Do not label deferred, uncertain or duplicate work done just to clear the list. Use only triage, in_progress and done. Deferred or waiting obligations stay in triage; explain the dependency in the note and use attention order for priority. Tasks use their lifecycle tools for start/completion; you must not start or complete one from this background assessment. A stalled Task may return to triage with evidence; otherwise leave its lifecycle status intact.",
    ...
    "- triage also includes an actual open obligation that can wait, has low consequence, or depends on someone else's next move. Explain what is waiting; never pretend it is completed. Reassess when new evidence changes its importance. Do not defer a consequential owned handoff just because it has no deadline or explicit question mark.",
