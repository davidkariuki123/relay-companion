...
// fresh credentials with the SAME shape `relay pair` writes, retain the packet
// store on a same-account reconnect (including read state), and wipe it when
// the identity actually changes. Then restart the daemon and the pill.
...
function readRelays() {
  ...
  return Object.entries(packets)
    ...
    .map(({ p, documents }) => ({
      ...
      taskAssignment: p.taskAssignment || null,
      ...
      recipientMentioned: p.recipientMentioned,
...
function rowById(packetId) {
  ...
  return {
    ...
    taskAssignment: sent.taskAssignment || null,
    taskRoster: Array.isArray(sent.taskRoster) ? sent.taskRoster : null,
    taskRosterCounts: sent.taskRosterCounts || null,
...
async function continuePacketSession(packetId, source = "relay") {
  ...
  // A Claude hand-off whose worker is still running has not been imported:
  // opening it now would hand Desktop a snapshot without the reply, and
  // Desktop never re-reads. The settle step imports it; then Open works.
  const handoff = deliveryRow.row?.agentHandoff;
  if (binding.provider === "claude" && handoff && handoff.imported === false && !deliveryRow.row?.workCompletedAt && !deliveryRow.row?.taskCompletedAt) {
    throw new Error("Claude Code shows this session as soon as the turn finishes.");
  }
  const observedBundle = await new Promise((resolve) => frontmostBundleId(resolve));
...
async function handOffToAgent(input) {
  ...
  if (firstTurn) {
    try {
      ...
      // Claude: Relay's worker owns the turn. Desktop never re-reads a
      // transcript it has already imported, so importing now would freeze a
      // snapshot without the reply (David, live, twice). The session is
  ...
  return { ok: true, provider: host, nativeId: String(binding.nativeId), taskStartedAt: stamped, routeId: id, imported: !claudeDeferred };
}
...
async function ensurePlainHandoffCompletionMonitor(relayId) {
  ...
  // Every hand-off now runs in a live session the reader can watch, so
  // completion is read from the transcript for both providers.
...
ipcMain.handle("relay:mentionVisit", async (event, input) => {
  ...
  if (!id.startsWith("grp_") || visitId.length < 8 || visitId.length > 128) return { ok:false };
  try {
    const client = await relayClient();
    return { ok:true, visit:await client.openMentionVisit(id, visitId) };
  } catch (error) {
    return { ok:false, error:error?.message || String(error) };
  }
});
